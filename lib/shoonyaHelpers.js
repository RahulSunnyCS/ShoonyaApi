...
const { loginShoonya } = require("../helper");
...
module.exports.init = async (api, autraData) => {
  const loginResult = await loginShoonya(api);
  const formattedDateWithYear = getFormattedDateWithYear();
  let optionChainDetails = [];
  // const expiryIndices = await this.getExpiryIndices(api);
  const expiryIndices = ["BANKNIFTY"];
  ...
  const indicesData  = autraData.createIndicesTokens(optionChainDetails);
  return {optionChainDetails, indicesData};
};
