...
var NorenRestApi = function (params) {
  ...
  self.login = async function (params) {
    ...
    try{
      let auth_data = await post_request("authorize", authparams);
      if (auth_data.stat == "Ok") {
        self.setSessionDetails(auth_data);
      }
      return auth_data;
    }catch(err){
      console.log(err)
   ...
   */
    ...
    self.get_option_chain = function (exch, tsym, strprc, cnt) {
      let values = {};
      values["uid"] = self.__username;
      values["exch"] = exch;
      values["tsym"] = tsym;
